...
\subsection{extpar\_sgsl\_to\_buffer}\label{extpar_topo_to_buffer}
...
The variables of the namelist 'INPUT\_SGSL', which cover all the raw subgrid-scale slope parameter data information, is fed into the program. In this namelist the path of the raw data is given as well as the names of the subgrid-scale slope data files. An integer switch allows to choose between the highly resolved, non-global DEM ASTER based and the coarser but global DEM GLOBE based raw data (1: GLOBE, 2: ASTER). In order to define the right number of raw data tiles the variables ntiles\_column and ntiles\_row must be available in the namelist. Additionally, the names for the buffer is defined.\par\medskip\noindent
...
Further the number of tiles of the raw topography data is defined (this depends on the DEM used:  1-16 tiles for GLOBE and 1 - 240 tiles for ASTER). This value is the product of ntiles\_column and ntiles\_row. The further processing follows exactly the one for the otographic height data.\par\medskip\noindent
\subsubsection{Used namelist files and data in-/output:}
\begin{itemize}
 \item namelists files: INPUT\_grid\_org, INPUT\_COSMO\_GRID, INPUT\_ICON\_GRID, INPUT\_GME\_GRID, INPUT\_SGSL
