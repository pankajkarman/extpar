...
\paragraph{Target grid definition} \ \par\medskip\noindent
The definition of the target grid is done using the namelist \textit{namelist.py}. \par\medskip\noindent
\paragraph{Raw NDVI data}\ \par\medskip\noindent
For the aggregation of the normalized differential vegetation index the namelist is quite simple again. It contains the path and the filename of the raw data set, as well as the names of the buffer and output files for the consistency check. Because the  consistency check requires a namelist readable for Fortran, \textit{extpar\_ndvi\_to\_buffer} generates that namelist under the name \textit{INPUT\_NDVI}. \par\medskip\noindent
\paragraph{Aggregation of the raw data to the target grid}\ \par\medskip\noindent
\textit{The following paragraphs describe computations on the raw data grid}\par\medskip\noindent
To aggregate the data to the target grid, the first-order conservative remapping operator \textit{remapycon} is used. \par\medskip\noindent 
\textit{The following paragraphs describe computations on the target grid}\par\medskip\noindent
After the remapping with CDO, in addition to the ndvi value, the monthly ratio with respect to the maximum ndvi is computed. \par\medskip\noindent
Finally the data is saved in a NetCDF buffer and an output file.  
\
\subsubsection{Used namelist files and data in-/output:}
\begin{itemize}
 \item namelist files: namelist.py
