...
extpar_lon = extpar_ei.createVariable('lon', np.float64, (
    'ke',
    'je',
    'ie',
))
...
extpar_lon[:, :, :] = np.rad2deg(
    np.reshape(ei.variables['clon'][:], (1, 1, cells)))

extpar_lat = extpar_ei.createVariable('lat', np.float64, (
    'ke',
    'je',
    'ie',
))
...
extpar_lat[:, :, :] = np.rad2deg(
    np.reshape(ei.variables['clat'][:], (1, 1, cells)))

extpar_ei_t_s = extpar_ei.createVariable('T_S', np.float32, (
    'time',
    'ke',
    'je',
    'ie',
...
extpar_ei_t_s[:, :, :, :] = np.reshape(ei.variables['T_S'][:, :],
                                       (12, 1, 1, cells))

extpar_ei_w_snow = extpar_ei.createVariable('W_SNOW', np.float32, (
    'time',
    'ke',
    'je',
    'ie',
...
extpar_ei_w_snow[:, :, :, :] = np.reshape(ei.variables['W_SNOW'][:, :],
                                          (12, 1, 1, cells))

extpar_ei_t_sea = extpar_ei.createVariable('T_SEA', np.float32, (
    'time',
    'ke',
    'je',
    'ie',
...
extpar_ei_t_sea[:, :, :, :] = np.reshape(ei.variables['T_SEA'][:, :],
                                         (12, 1, 1, cells))
