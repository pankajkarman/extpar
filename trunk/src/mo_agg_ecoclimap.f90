...
MODULE mo_agg_ecoclimap
  ...
  USE mo_ecoclimap_lookup_tables, ONLY: ecoclimap_look_up
    ...
    USE mo_target_grid_data, ONLY: search_res !< resolution of ICON grid search index list
     ...
     REAL (KIND=wp), INTENT(OUT)  :: ecoclimap_class_fraction(:,:,:,:)  !< fraction for each ecoclimap class on target grid (dimension (OCLIMAP nclass',  nclass_ecoclimap


    INTEGER (KIND=i8), INTENT(OUT) :: ecoclimap_class_npixel(:,:,:,:) !< number of raw data pixels for each ecoclimap class on target grid (dimension (ie,je,ke,nclass_ecoclimap))


    INTEGER (KIND=i8), INTENT(OUT) :: ecoclimap_tot_npixel(:,:,:)  !< total number of ecoclimap raw data pixels on target grid (dimension (ie,je,ke))
    ...
    REAL (KIND=wp), INTENT(OUT)  :: emissivity_ecoclimap(:,:,:) !< longwave emissivity due to ecoclimap land use da
     ...
     REAL (KIND=wp)    :: a_class(1:tg%ie,1:tg%je,1:tg%ke,1:nclass_ecoclimap) !< area for each land use class grid  in target grid element (for a area weight)
