...
sys.path.append(
    os.path.join(os.path.dirname(__file__),
                 "./tools"))  # this is the generic folder for subroutines
...
def parse_config_file(filename):
    ...
    try:
        ...
        conf.dual_params = ast.literal_eval(
            config.get('ts_config', 'dual_params'))
...
def parse_cmdline():
    ...
    parser.add_option(
        ...
        dest="nprocs",
        help=(
            "number of processors (nprocx*nprocy+nprocio) to use [default=%d]"
            % DefaultValues.nprocs))
    ...
    parser.add_option(
        ...
        dest="nprocio",
        help=
        "set number of asynchronous IO processor, [default=<from namelist>]")
    ...
    parser.add_option("-v",
                      type="int",
                      dest="v_level",
                      help=("verbosity level 0 to 3 [default=%d]" %
                            DefaultValues.v_level))
    ...
    parser.add_option(
        ...
        dest="mpicmd",
        type="string",
        help=("MPI run command (e.g. \"mpirun -n\") [default=\"%s\"]" %
              DefaultValues.mpicmd))
    ...
    parser.add_option("--force-match",
                      dest="forcematch",
                      action="store_true",
                      default=(DefaultValues.forcematch == 1),
                      help="Force bit-reproducible results")

    parser.add_option("--tune-thresholds",
                      dest="tune_thresholds",
                      action="store_true",
                      default=DefaultValues.tune_thresholds,
                      help="Change thresholds to always at least return OK")

    parser.add_option("--update-thresholds",
                      dest="update_thresholds",
                      action="store_true",
                      default=DefaultValues.update_thresholds,
                      help="Update the thresholds")

    parser.add_option("--tuning-iterations",
                      dest="tuning_iterations",
                      action="store",
                      default=DefaultValues.tuning_iterations,
                      help="Defines how many times the tuning gets executed")

    parser.add_option("--reset-thresholds",
                      dest="reset_thresholds",
                      action="store_true",
                      default=DefaultValues.reset_thresholds,
                      help="Set all thresholds to 0.0 before tuning")

    # update namelist (no run). This is useful to quickly change all namelist at once
    parser.add_option("--update-yufiles",
                      dest="upyufiles",
                      action="store_true",
                      default=False,
    ...
    parser.add_option("-l",
                      ...
                      type="string",
                      action="store",
                      default=DefaultValues.testlist,
                      help=("Select the testlist file [default=%s]" %
                            DefaultValues.testlist))
    ...
    parser.add_option("-t",
                      ...
                      action="store",
                      default=None,
                      help=("Timeout in s for each test [default=%s]" %
    ...
    parser.add_option("--tolerance",
                      ...
                      type="string",
                      action="store",
                      default=DefaultValues.tolerance,
                      help=("Select the tolerance file name [default=%s]" %
    ...
    try:
        ...
        sys.exit(
            "problem parsing command line arguments (check ./testsuite.py -h for valid arguments)"
        )
...
def main():
    ...
    if os.path.isfile("./testsuite_config.cfg"):
        config_file = "./testsuite_config.cfg"
            ...
            os.path.join(os.path.dirname(__file__), "./testsuite_config.cfg")):
        config_file = os.path.join(os.path.dirname(__file__),
                                   "./testsuite_config.cfg")
    else:
    ...
    status = system_command('/bin/mkdir -p ' + options.workdir + '/',
                            logger,
        ...
        if mytest.run_test():
            ...
            try:
                ...
                #
                    ...
                    logger.important(
                        'Updating the thresholds on the current runs')
                ...
                # if upnamelist=True, no model run.
                    ...
                    if (mytest.options.tune_thresholds):
                        ...
                        for i in range(int(mytest.options.tuning_iterations)):
                            ...
                            logger.important("Iteration number {0}".format(i +
                                                                           1))
                            mytest.prerun(
