...
def main():
    ...
    for i in range(len(lphys)):
        ...
        for fname in l_files:
            ...
            if fname == 'INPUT_ORG':
                change_line(new_fname, ' lphys =',
                            '  lphys = ' + lphys[i] + ',\n')
                change_line(new_fname, ' luseobs =',
                            '  luseobs = ' + luseobs[i] + ',\n')
                change_line(new_fname, '$info', '! $info = ' + info[i] + '\n')
            elif fname == 'INPUT_PHY':
                change_line(new_fname, ' lgsp =',
                            '  lgsp = ' + lgsp[i] + ',\n')
                change_line(new_fname, ' lrad =',
                            '  lrad = ' + lrad[i] + ',\n')
                change_line(new_fname, ' lconv =',
                            '  lconv = ' + lconv[i] + ',\n')
                change_line(new_fname, ' ltur =',
                            '  ltur = ' + ltur[i] + ',\n')
                change_line(new_fname, ' lsoil =',
                            '  lsoil = ' + lsoil[i] + ',\n')
...
def change_line(filename, identifier, newline):
    ...
    if counter != 1:
        print('Error: %s appears %i times in file %s' %
              (identifier, counter, filename))
        print(
            'Note: check that white space are not missing in corresponding line %s'
            % (identifier))
        sys.exit()
