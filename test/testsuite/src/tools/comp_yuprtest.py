...
def cmp_(file1,file2, \
    ...
    while True:
        ...
        if (i1 >= len(data1)) or (i2 >= len(data2)):
            print(
                'Files %s and %s do not have overlapping time steps and can not be compared.'
                % (file1, file2))
            return -1
    ...
    while True:
        ...
        if (int(l1[1]) != ntstep) or leof:
            ...
            if (lerror_t) and (v_level == 1):
                if print_header:
                    ...
                    print_header = False
                print(
                    'nt=%i, max rel. er. t,p: %1.1e above threshold %1.1e, at line %i'
                    % (ntstep, maxdiff_t, tol_t, lnum_t))
            ...
            if (lerror_a) and (v_level == 1):
                if print_header:
                    print(header)
                    print_header = False
                print 'nt=%i, max rel. er. all: %1.1e above threshold %1.1e, at line %i' % (
                    ntstep, maxdiff_a, tol_a, lnum_a)
        ...
        for j in range(len(l1)):
            ...
            if is_num(l1[j]):
                ...
                if minval == -1 and is_not_int(
                        n1):  #note: int are not considered (min-max index)
                    ldiff = abs(n1 - n2)
                #relative diffference
                elif abs(n1) > minval and is_not_int(
                        n1):  #note: int are not considered (min-max index)
                    ldiff = abs((n1 - n2) / n1)
                else:
                    ldiff = 0
