...
def default_plot_creation():
    for filename in glob.iglob(args.inputFiles[0] + "/*"):
        ...
        old_thresholds = ts_thresholds.Thresholds(args.inputFiles[0] +
                                                  filename)
        new_thresholds = ts_thresholds.Thresholds(args.inputFiles[1] +
                                                  filename)
    ...
    for file in files:
        ...
        for key, value in threshold_dict.items():
            if key not in ["minval", "steps", "CHKDAT"]:
                print("dbg: " + str(value) + " " +
                      str(threshold_dict["steps"]))
                plot_data[key] = pd.Series(value,
                                           index=threshold_dict["steps"])
...
def _plot(plot_data, title, index=""):
    ...
    my_colors = list(
        islice(
            cycle([
                'DarkGreen', 'DarkBlue', 'Yellow', 'Gray', 'DarkOrange', "Red",
                "Pink", "LightBlue", "Purple", "Pink"
            ]), None, len(plot_data)))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="A tool to create linegraphs of two \
    ...
    parser.add_argument(
        ...
        help="define the destination folder for the generated pictures")
    args = parser.parse_args()
    os.makedirs(args.outputFolder, exist_ok=True)
    if (len(args.inputFiles) == 2) and (os.path.isdir(
            args.inputFiles[0])) and (os.path.isdir(args.inputFiles[1])):
