...
MODULE mo_globcover_tg_fields
  ...
  ! &                                    globcover class on target grid (dimension (ie,je,ke,nclass_globcover))
            &                            globcover_tot_npixel(:,:,:)  !< total number of globcover raw data pixels &
  ...
  ! &                                  on target grid (dimension (ie,je,ke,nclass_globcover))
            ...
            &                          emissivity_globcover(:,:,:) !< longwave emissivity due to globcover land use data
  ...
  SUBROUTINE allocate_globcover_target_fields(tg)
    ...
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array fr_land_globcover',__FILE__,__LINE__)
    fr_land_globcover = 0.0

    ALLOCATE (globcover_tot_npixel(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array globcover_tot_npixel',__FILE__,__LINE__)
    globcover_tot_npixel = 0

    ALLOCATE (globcover_class_fraction(1:tg%ie,1:tg%je,1:tg%ke,1:nclass_globcover), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array globcover_class_fraction',__FILE__,__LINE__)
    globcover_class_fraction = 0.0


    ALLOCATE (globcover_class_npixel(1:tg%ie,1:tg%je,1:tg%ke,1:nclass_globcover), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array globcover_class_npixel',__FILE__,__LINE__)
    globcover_class_npixel = 0

    ALLOCATE (ice_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array ice_globcover',__FILE__,__LINE__)
    ice_globcover = 0.0

    ALLOCATE (z0_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array z0_globcover',__FILE__,__LINE__)
    z0_globcover = 0.0

    ALLOCATE (root_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array root_globcover',__FILE__,__LINE__)
    root_globcover = 0.0

    ALLOCATE (plcov_mx_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array plcov_mx_globcover',__FILE__,__LINE__)
    plcov_mx_globcover = 0.0


    ALLOCATE (plcov_mn_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array plcov_mn_globcover',__FILE__,__LINE__)
    plcov_mn_globcover = 0.0

    ALLOCATE (lai_mx_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array lai_mx_globcover',__FILE__,__LINE__)
    lai_mx_globcover = 0.0

    ALLOCATE (lai_mn_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array lai_mn_globcover',__FILE__,__LINE__)
    lai_mn_globcover = 0.0

    ALLOCATE (rs_min_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array rs_min_globcover',__FILE__,__LINE__)
    rs_min_globcover = 0.0

    ALLOCATE (urban_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array urban_globcover',__FILE__,__LINE__)
    urban_globcover = 0.0

    ALLOCATE (for_d_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array for_d_globcover',__FILE__,__LINE__)
    for_d_globcover = 0.0

    ALLOCATE (for_e_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array for_e_globcover',__FILE__,__LINE__)
    for_e_globcover = 0.0

    ALLOCATE (skinc_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array skinc_globcover',__FILE__,__LINE__)
    skinc_globcover = 0.0

    ALLOCATE (emissivity_globcover(1:tg%ie,1:tg%je,1:tg%ke), STAT=errorcode)
    IF(errorcode.NE.0) CALL logging%error('Cant allocate the array emissivity_globcover',__FILE__,__LINE__)
    emissivity_globcover = 0.0

    CALL logging%info('Exit routine: allocate_globcover_target_fields')
