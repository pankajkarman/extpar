...
MODULE mo_agg_flake
  ...
  USE mo_flake_data, ONLY: flake_grid, &
    ...
    USE mo_target_grid_data, ONLY: lon_geo, & !< longitude coordinates of the COSMO grid in the geographical system 
     ...
     apix_e  = re * re * deg2rad* ABS(flake_grid%dlon_reg) * deg2rad * ABS(flake_grid%dlat_reg) ! area of GLCC raw data pixel at equator
     ...
     !rows: DO j_row=1,100
       ...
       columns: DO i_col=1, flake_grid%nlon_reg
